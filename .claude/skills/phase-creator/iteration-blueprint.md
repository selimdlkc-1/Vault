...
Her faz skill'indeki **tek bir iterasyonun** zorunlu yapısı. Amaç: geliştirici agent'ın **Plan moduna geçmeden** aynı netlikte çalışması; spec `Docs/` içinde kalır, iterasyon **ne yapılacağını** ve **hangi doc bölümünün neden okunacağını** taşır.
...
**Dosya yerleşimi:** Faz 5+ iterasyon içeriyorsa her iterasyon `iterations/MM-<slug>.md` dosyasına yazılır ve `SKILL.md`'deki indeks tablosundan işaret edilir. Agent yalnızca çalıştığı iterasyonun dosyasını okur — faz skill'i tetiklendiğinde 9 iterasyon birden context'e girmez.
...
**Kalite kapıları:** (`Docs/08_TESTING_STRATEGY.md` + `.claude/rules/04-quality-gates.md`)
- [ ] Pozitif senaryo testi
- [ ] En az bir deny testi (RBAC / validation / parse hatası)
- [ ] Lint + type check + ilgili test komutu yeşil
- [ ] Coverage: <modül> ≥<% hedef>
...
**Tekrarlama:** Convention ve eşikler `.claude/rules/` içinde zaten yüklü — iterasyonda tekrar yazma, gerekiyorsa dosya adıyla referans ver.
...
**Uygulama planı:**
1. `schemas/auth.py` — `LoginRequest`, `TokenResponse`; `Docs/03` §2.1 alanları birebir
...
- [ ] Auth modülü coverage ≥%90 (`.claude/rules/04-quality-gates.md`)
...
## Doğrulama (iterasyon başına)

- [ ] 11 zorunlu bölüm dolu (Hedef … Stop)
...
- [ ] `.claude/rules/` içeriği tekrarlanmamış, referans verilmiş
