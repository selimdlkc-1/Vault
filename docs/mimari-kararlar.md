...
- Yalnızca `draft` state'indeki transfer kullanıcı tarafından silinebilir/vazgeçilebilir; diğer terminal-olmayan durumlarda iptal yoktur. Bu silme (`DELETE /transfers/:id`), taslağın tek `null → draft` denetim izi kaydını da aynı transaction'da kaldırır — `transfer_state_events`'in append-only garantisi ve FK `RESTRICT`'i (`docs/02_DATABASE_SCHEMA.md` §2.8/§7) `draft`'ı geçmiş transferlerin izini korur, zincire hiç dokunmamış bir taslak için geçerli değildir. `Admin` bu mutasyondan muaf değildir (salt-okunur erişimi yalnızca `GET`'i kapsar). Netleştirme Faz 5 §5.6b'de yapıldı (bkz. Versiyon Geçmişi 0.7).
...
| 0.7 | 2026-09-01 | W-005 netleştirildi ve `GET /transfers/:id` + `DELETE /transfers/:id` uç sözleşmeleri somutlaştırıldı. Faz 5 §5.6b (Frontend: S-TRANSFER-CONFIRM + S-TRANSFER-DETAIL) uygulanırken tespit edildi: `docs/03_API_CONTRACTS.md` §5.4 bu iki ucu zaten tanımlıyordu ama hiçbir backend iterasyonu implemente etmemişti (§5.6a yalnızca `POST /transfers`'ü tüketmişti), oysa detay/onay ekranı ikisine de bağımlı. Kullanıcı onayıyla §5.6b tek full-stack PR olarak teslim edildi (roadmap alt madde numarası değişmedi). Ayrıca bir spec gerilimi çözüldü: `transfer_state_events` append-only'dir (`docs/02` §2.8) ve FK `RESTRICT`'tir (§7), ama W-005 `draft` transferin silinebileceğini söylüyordu — bir taslağın tek `null → draft` kaydının, taslakla **birlikte** aynı transaction'da silinmesi netleştirildi (bağımsız silme değil; `draft`'ı geçmiş transferlerin izi korunur). Yeni bir mimari karar değil, W-005 + §5.4'te zaten kararlaştırılmış davranışın kod/şema karşılığının tamamlanması. `docs/02` §2.8, `docs/03` §5.4/§6, `docs/10` §5.6 güncellendi. |
