...
> **Versiyon:** 0.5 (Taslak — tüm çekirdek bölümler kapandı)
...
- `transfers`: `id, walletId (FK, managed), networkId, assetId, toAddress, amount (string, en küçük birim), state (enum), txHash (nullable), failureReason (nullable), createdAt, updatedAt`.
