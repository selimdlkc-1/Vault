...
**§4.3 — Frontend: S-WALLET-ADD-MANAGED.** Yönetilen cüzdan oluşturma formu ve akışı. 🔜 Kod tamam — `feat/frontend-add-managed-wallet`: `/wallets/add?type=managed` dalı (tek alan: Ağ), `useCreateManagedWallet`, "Oluşturuluyor..." bekleme state'i, başarıda `/wallets/[id]` yönlendirmesi + "Yönetilen cüzdanınız oluşturuldu." toast'ı (yeni minimal `ToastProvider`), `NETWORK_ASSET_INACTIVE` alan-altı hata; `/wallets/add` (type'sız) artık tip seçim ekranı. Manuel tarayıcı doğrulaması kullanıcı onayında.

**§4.4 — Mock kontrat deploy ve admin mint.** Mock ERC-20/TRC-20 kontratlarının Sepolia/BSC Testnet/Tron Shasta'ya deploy edilmesi (deploy script'i, kontrat adreslerinin `assets.contract_address`'e yazılması); `mint_operations` tablosu; `POST /admin/mint`; S-ADMIN-MINT ekranı. Kapsamın genişliği (yeni bir `packages/contracts` workspace'i + backend endpoint'i + frontend ekranı — üç farklı katman) nedeniyle bu alt madde üç iterasyona bölünür: mock kontrat + deploy altyapısı (§4.4a — `docs/mimari-kararlar.md` TS-008), `mint_operations` + `POST /admin/mint` + `GET /admin/users` (kullanıcı arama, daha önce hiçbir fazda tanımlanmamıştı) (§4.4b), S-ADMIN-MINT ekranı (§4.4c). S-ADMIN-MINT'in kullanıcı/cüzdan seçim akışı, Faz 3 §3.4a'da zaten Admin-farkında (`?userId=`) olarak teslim edilmiş `GET /wallets` endpoint'ini kullanır (`docs/03_API_CONTRACTS.md` §5.2) — Faz 6 §6.4'ten hiçbir endpoint öne çekilmez, bkz. Faz 6 §6.4 notu. 🔜 §4.4a kod tamam — `feat/mock-contract-deploy-infra`: yeni `packages/contracts` workspace'i (Hardhat + TS, `apps/api`'nin runtime bağımlılığı değil — TS-008 / `docs/adr/0001`), `MockERC20.sol` (OZ `ERC20`+`Ownable`, `mint() onlyOwner`), `scripts/{deploy-evm,deploy-tron,write-contract-addresses,token-catalog}.ts`. Üç ağa gerçek deploy + `assets.contract_address` yazımı, kullanıcının fonlanmış testnet cüzdanı/RPC anahtarlarıyla manuel adım (onayda). §4.4b kod tamam — `feat/admin-mint-endpoint` (`11b63e6`): `mint_operations` tablosu + migration, `POST /admin/mint` (cüzdan/varlık doğrulama → provider `mintToken()` → atomik `mint_operations` insert + `MINT_EXECUTED` audit), `GET /admin/users` (kullanıcı arama), `admin/` modülü ilk kez oluşturuldu, mint rate limit 20/dk. §4.4c kod tamam — `feat/frontend-admin-mint`: `/admin/mint` ekranı (S-ADMIN-MINT), `useAdminUserSearch` (debounce'lu e-posta araması) + `useAdminUserWallets` (`GET /wallets?userId=`) + `useMint` hook'ları, kademeli alan aktifleşmesi (Kullanıcı → Cüzdan → Varlık → Tutar), istemci `BigInt` tutar dönüşümü, `CHAIN_PROVIDER_UNAVAILABLE`/`RESOURCE_NOT_FOUND` hata eşlemesi, oturum-içi "Son Mint İşlemleri" listesi, admin nav'a "Mock Mint" linki.

**İnsan onay noktası:** Faz 4 sonunda bir yönetilen cüzdan oluşturulabiliyor, private key'in hiçbir katmanda düz metin olarak sızmadığı (log, API yanıtı, DB) manuel ve otomatik testle doğrulanmış olmalı — bu, projenin en hassas güvenlik kontrol noktasıdır ve kullanıcı onayı özellikle bu noktada beklenir. 🔜 Kod tamam (§4.1–§4.4c) — private key sızıntısı otomatik testle §4.2'de (`feat/hd-wallet-managed-endpoint`) doğrulandı; manuel kod incelemesi (log/API yanıtı/DB'de çözülmüş key yok) + managed cüzdan oluşturma ve `/admin/mint` → bakiye artışı tarayıcı doğrulaması kullanıcı onayında yapılır, onaylanınca bu satır ✅ olur ve Faz 5 başlar.
...
**§5.6 — Frontend: transfer akışı.** S-TRANSFER-NEW, S-TRANSFER-CONFIRM, S-TRANSFER-DETAIL (5 saniyelik polling, 8 durumun TR badge karşılığı); hareket geçmişinde tekilleştirme (`chain_movements` ile `txHash` eşleşmesi). Kapsamın genişliği (3 ekran, farklı alan listesi ve UX state'leri) nedeniyle bu alt madde iki iterasyona bölünür: draft oluşturma formu (§5.6a — S-TRANSFER-NEW) ve onay + izleme (§5.6b — aynı `/transfers/[id]` route'unun iki alt görünümü olan S-TRANSFER-CONFIRM ve S-TRANSFER-DETAIL, tekilleştirme dahil). 🔜 §5.6a kod tamam — `feat/frontend-transfer-new` (#39): `/transfers/new` (S-TRANSFER-NEW), `useCreateTransfer` + kademeli alan aktifleşmesi + istemci `BigInt` tutar dönüşümü, `?walletId=` ön-doldurma. §5.6b kod tamam — `feat/frontend-transfer-confirm-detail`: `/transfers/[id]` tek route'unda `state === 'draft'` iken S-TRANSFER-CONFIRM (step-up formu + "İptal Et"), aksi halde S-TRANSFER-DETAIL (5 sn polling, terminal durumda durur, `TransferStateBadge` merkezi 8 durum eşlemesi, denetim izi zaman çizelgesi, `dropped → "Yeniden Dene"` query-param'lı yeni taslak). Bu iterasyonda `docs/03` §5.4'te tanımlı ama implemente edilmemiş `GET /transfers/:id` (detay + `stateEvents`, Admin salt-okunur) ve `DELETE /transfers/:id` (yalnızca sahibinin `draft`'ı) backend uçları da eklendi — kullanıcı onayıyla tek full-stack PR (`docs/mimari-kararlar.md` Versiyon Geçmişi 0.7). Hareket geçmişinde `source: 'system'` birleşimi/tekilleştirme Faz 6'ya kaldı (bu iterasyon yalnızca `TransferStateBadge`'i tanıttı).
